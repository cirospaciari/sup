    ...
    var _cloneInterationDOM = function(element, elementToClone){
        ...
        for(var attributeIndex = 0; attributeIndex < elementToClone.attributes.length; attributeIndex++){
            cloned.attributes[attributeIndex]  = {
                type:  elementToClone.attributes[attributeIndex].type,
                attribute: elementToClone.attributes[attributeIndex].attribute,
                code:  elementToClone.attributes[attributeIndex].code,
                oldValue: elementToClone.attributes[attributeIndex].oldValue,
                bindAttribute: elementToClone.attributes[attributeIndex].bindAttribute,
                outerHTML: elementToClone.attributes[attributeIndex].outerHTML,
                lastDOMValue: elementToClone.attributes[attributeIndex].lastDOMValue
            };
        }
        for(var clonedIndexChild = 0; clonedIndexChild < elementToClone.childs.length; clonedIndexChild++){;
            cloned.childs[clonedIndexChild] = _cloneInterationDOM(element.childNodes[elementToClone.childs[clonedIndexChild].originalChildIndex], 
                                                                 elementToClone.childs[clonedIndexChild]);
        }
            ...
            if(attribute.type == "loop"){
                ...
                if(attribute.loaded && virtualDOM.loadedLoopItens){
                    ...
                    if(attribute.isRange){
                        if(attribute.rangeInit != newLoopData.rangeInit || attribute.rangeEnd != newLoopData.rangeEnd)
                        {
                            ...
                            hasChangedSize = attribute.rangeEnd - attribute.rangeInit != newLoopData.rangeEnd - newLoopData.rangeInit;
                        }
                    }else{
                        hasChangedValues = attribute.loopArray != newLoopData.loopArray;
                        hasChangedSize = (attribute.loopArray || []).length != (newLoopData.loopArray || []).length;
                    }
                    ...
                    hasChangedValues = true;
                    hasChangedSize = true;
                    console.timeEnd("runLoop");
                }
                var newsItensFragment = null;
                
                
                if(hasChangedSize){
                    var newTotalItens = attribute.isRange ?  attribute.rangeEnd - attribute.rangeInit + 1 : (attribute.loopArray || []).length;
                    //remove elements and virtualsDOMS
                    while(newTotalItens < virtualDOM.loadedLoopItens.length){
                    ...
                    if(newTotalItens > virtualDOM.loadedLoopItens.length){
                       
                        var newElementList = new Array(newTotalItens - virtualDOM.loadedLoopItens.length);
                        console.time("cloneDOM");
                        for(var k = 0; k < newTotalItens - virtualDOM.loadedLoopItens.length; k++){
                            virtualDOM.element.insertAdjacentHTML('beforeBegin', attribute.originalInterationOuterHTML || virtualDOM.outerHTML);
                            newElementList[k] = virtualDOM.element.previousElementSibling;
                            if(!attribute.originalInterationDOM){
                                attribute.originalInterationDOM = _parser(newElementList[0]);
                                attribute.originalInterationOuterHTML = newElementList[0].outerHTML;
                            }
                        }
                        console.timeEnd("cloneDOM");
                        
                        if(newElementList.length > 0){
                            console.time("cloneVirtualDOM");
                            ...
                            for(var newItemFragmentIndex = 0; newItemFragmentIndex < newElementList.length; newItemFragmentIndex++)
                            {
                                var newDomContext = _cloneContext(context);
                                var clonedInterationDOM =  _cloneInterationDOM(newElementList[newItemFragmentIndex], attribute.originalInterationDOM);
                                virtualDOM.loadedLoopItens.push( { item: clonedInterationDOM, context: newDomContext });   
                            }
                            //context.pop();
                            console.timeEnd("cloneVirtualDOM");
                        }
                    }
                }
                ...
                if(virtualDOM.loadedLoopItens.length){
                    ...
                    for(var loadedItemIndex = 0; loadedItemIndex < virtualDOM.loadedLoopItens.length; loadedItemIndex++){
                        ...
                        if(hasChangedValues){
                            var value = attribute.isRange ? (loadedItemIndex + attribute.rangeInit) : attribute.loopArray[loadedItemIndex];
                            if(attribute.loopIndexVariable){
                                virtualDOM.loadedLoopItens[loadedItemIndex].context.values[attribute.loopIndexIndex] = loadedItemIndex;
